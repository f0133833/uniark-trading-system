...
如果已经能跑出一张带 MACD 的 K 线图，可关掉本文档，直接去主仓库读源码。
...
> **读理论和读代码不是"学习"，是"准备学习"。真正的学习从写出第一行代码开始。**

如果已经在"再看一会就开始"的状态里待了 3 天以上，那很可能不是知识不够，而是开始的方式不太对。

### 为什么会卡住

原因在于：目标被定成了"做一个交易系统"。

这个目标对大脑来说**太大、太抽象**，于是大脑停在了原地踏步——读资料。

让 AI "帮我读这个开源项目"是同样的陷阱。AI 总结读得越多，越觉得"我懂了"，但代码却还没写出一行。
...
这张图出来的那一刻，就跨过了从"读理论的人"变成"有代码的人"的鸿沟。
...
AI 会给出一堆架构图、概念解释、3000 行的"完整方案"。看完依然不知道该写什么。
...
AI 给的就是 20 行可运行的代码。复制粘贴，跑一下，要么过，要么改两行就过。

### 黄金法则

> **把"做交易系统"切成 N 个"写一个 20 行函数"的问题。**

工作不再是"理解 + 实现"，而是"验证 + 拼装"。
...
- 新建 `data.py`，让 AI 写出 `get_weekly_klines`
...
每一件都是无底洞，每一件都能让进度卡住三天。**全部在 MVP 跑通之后再考虑。**
...
4. **三段结构识别** ← 这才是真正"自己的理论"部分，AI 帮不上太多忙
...
最理想的用法：先按前面的五天计划自己写一版，跑通之后再回头对照——会发现自己写的版本和这里的差距其实没那么大，**这种"我也能写出这种代码"的觉察，比直接抄过来用更有价值**。

如果卡在某个具体细节（比如 `pd.to_datetime` 该怎么传参、`mplfinance` 怎么加副图），也可以来这里查具体的某一段。
...
应该弹出一张 BTC 周线 + 均线 + 成交量 + MACD 的组合图。看到这张图的那一刻，整个 MVP 就完成了——已经从"读理论的人"变成"有代码的人"。
