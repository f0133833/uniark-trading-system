...
`ratio_threshold=0.5` 这个魔数从哪来？是项目作者在加密货币周线/日线上的经验值。日线想更严可以调 0.3~0.4，4 小时以下噪声大可以调 0.6~0.7。它不是公理，是工程参数。
...
最终版加了一个独立的补检函数，专门处理"hist 翻号后才出现的价格极值"。它的工作方式（简化描述）：

- 扫描每段 hist 同向段
- 检查段**两侧的几根反向 hist 柱**对应的价格里，是否出现了"比段内极值更极端"的价格
- 如果有，按 `level=0` 生成一条"极值背离"记录（用 level=0 区分于标准三段背离）

最终版的入口函数会同时调用 `find_three_segment_divergences` 和 `find_missed_extremes`，结果合并。

### 要不要补

**如果交易体系依赖"不漏掉任何真实顶/底"，必须补。** 这种情况下建议直接切换到最终版，不要自己实现——这个补丁的边界情况很多（"两侧扫多远"、"价格阈值怎么定"），自己写容易越补越乱。

如果能接受"标准三段背离会漏掉一部分极值，但保证查全的那些都是结构清晰的"——可以不补，先用初版。
